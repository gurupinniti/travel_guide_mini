    ...
    <div className="flex flex-col h-screen max-w-4xl mx-auto bg-gray-50">
      ...
      <div className="flex-1 p-4 overflow-y-auto bg-white">
        {conversations.map((msg, index) => (
          <div key={index} className={`flex items-start mb-4 ${msg.role === 'user' ? 'justify-start' : 'justify-end'}`}>
            <div 
              className={`max-w-xs p-4 rounded-lg ${msg.role === 'user' 
                ? 'bg-blue-100 text-left'  // User message with lighter color
                : msg.role === 'system'
                ? 'bg-gray-200 text-left'  // System message
                : 'bg-blue-600 text-white'}  // Assistant message with bright blue
              `}
            >
              <div className="flex items-center mb-1">
                <span className="font-semibold text-sm">{msg.role === 'user' ? 'You ' : msg.role === 'assistant' ? 'Assistant' : 'System'}</span>
